...
export class IPFS extends Component {
    ...
    addFile = async (e) => {
        ...
        await ipfs.add(this.state.buffer, (err, result) => {
            ...
            //adding new item 
            const newItem = <a href={this.state.fileLink} target="_blank"><span className="descr">FileName :</span>{this.state.fileName} <br /> <span className="descr">Description :</span>{this.state.text} <br /> <span className="descr">FileHash :</span>{this.state.fileHash}</a>
...
<div class="area" >
    ...
    <div>
                ...
                <form className="box-form" onSubmit={this.addFile}>
                    ...
                    <input id="box-descr" type="text" placeholder="Enter some description for your file" value={this.state.text} onChange={this.handleInput} />
                    <input type="submit" id="sub-but" />
                    <br />
                    {this.state.toAdd ? <span>You can add a file now</span> : <span>Please wait the file is being uploaded</span>}
                </form>

                <div className="uploaded">
                    <h1>Dropped files</h1>
                    <ul>
                        {this.state.items.map(item => <li key={this.props.id}>{item}</li>)}
                    </ul>
                </div>
